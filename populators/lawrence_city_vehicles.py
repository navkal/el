...
# Combine '_x' and '_y' columns generated by merge, giving preference to column '_y'
def combine_columns( df, col_name ):

    df = df.rename( columns={ col_name + '_y': col_name } )
    df[col_name] = df[col_name].combine_first( df[col_name + '_x'] )
    df = df.drop( columns=[col_name + '_x'] )
...
if __name__ == '__main__':
    ...
    # Replace years represented as 0
    df_atr[util.MODEL_YEAR] = df_atr[util.MODEL_YEAR].replace( 0, np.nan )
    ...
    df = combine_columns( df, util.MODEL_YEAR )
    df = combine_columns( df, util.MODEL )
    df = combine_columns( df, util.DEPARTMENT )
    df = combine_columns( df, util.MAKE )

    # Change year column from float to int
    df[util.MODEL_YEAR] = df[util.MODEL_YEAR].astype( int )
