...
def analyze_contractor_activity( df, table_name, project_type ):
    ...
    for idx, df_group in df_permits.groupby( by=[util.APPLICANT, util.YEAR] ):

        ROW[util.YEAR] = df_group.iloc[0][util.YEAR]
        ROW[util.APPLICANT] = df_group.iloc[0][util.APPLICANT]
...
if __name__ == '__main__':
    ...
    df = analyze_contractor_activity( df, 'BuildingPermits_L_Wx', 'wx' )
    df = analyze_contractor_activity( df, 'BuildingPermits_L_Solar_Summary', 'solar' )
    ...
    util.create_table( 'BuildingContractorActivity', conn, cur, df=df )
